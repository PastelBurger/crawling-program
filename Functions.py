...
def pt_trend (url, df) :
    ...
    for tag in s2["대표출원인"]:
        ...
        for 국가 in 국가코드:
            ...
            # Y축의 눈금 설정
            y_min = int(min(개수_합계2))
            y_max = int(max(개수_합계2))
            y_range = y_max - y_min
            if y_range > 1000:
                plt.yticks(range(y_min, y_max + 1, 50))  # 눈금 간격이 4인 경우
            elif y_range > 500:
                plt.yticks(range(y_min, y_max + 1, 20))  # 눈금 간격이 2인 경우
            elif y_range > 150:
                plt.yticks(range(y_min, y_max + 1, 10))  # 눈금 간격이 2인 경우
            elif y_range > 60:
                plt.yticks(range(y_min, y_max + 1, 5))  # 눈금 간격이 2인 경우
            elif y_range > 20:
                plt.yticks(range(y_min, y_max + 1, 2))  # 눈금 간격이 2인 경우
            else:
                plt.yticks(range(y_min, y_max + 1))  # 눈금 간격이 1인 경우
            plt.grid(True, axis='y', alpha=0.5, linestyle='--')
